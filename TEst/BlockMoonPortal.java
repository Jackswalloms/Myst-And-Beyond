...
public class BlockMoonPortal extends BlockBreakable
{
    ...
    public static boolean tryToCreatePortal(World world, int i, int j, int k)
    {
        int l = 0;
        int i1 = 0;
        if(world.getBlockId(i - 1, j, k) == Block.cobblestone.blockID || world.getBlockId(i + 1, j, k) == Block.cobblestone.blockID)
        {
            l = 1;
        }
        if(world.getBlockId(i, j, k - 1) == Block.cobblestone.blockID || world.getBlockId(i, j, k + 1) == Block.cobblestone.blockID)
        {
            i1 = 1;
        }
        if(l == i1)
        ...
        for(int j1 = -1; j1 <= 2; j1++)
        {
            for(int l1 = -1; l1 <= 3; l1++)
            {
                ...
                if(flag)
                {
                    if(j2 != Block.cobblestone.blockID)
                    {
                        return false;
                    }
                    continue;
                }
                if(j2 != 0 && j2 != mod_moon.moonfire.blockID)
                {
                    return false;
                }
            }
    ...
    public void onNeighborBlockChange(World world, int i, int j, int k, int l)
    {
        ...
        if(world.getBlockId(i, k1 - 1, k) != Block.cobblestone.blockID)
        {
        ...
        if(l1 != 3 || world.getBlockId(i, k1 + l1, k) != Block.cobblestone.blockID)
        {
            world.setBlockWithNotify(i, j, k, 0);
            return;
        }
        ...
        if((world.getBlockId(i + i1, j, k + j1) != Block.cobblestone.blockID || world.getBlockId(i - i1, j, k - j1) != blockID) && (world.getBlockId(i - i1, j, k - j1) != Block.cobblestone.blockID || world.getBlockId(i + i1, j, k + j1) != blockID))
        {
            world.setBlockWithNotify(i, j, k, 0);
            return;
        } else

...
public class BlockMoonPortal extends BlockBreakable
{
    ...
    public BlockMoonPortal(int i, int j)
    {
    ...
    public static boolean tryToCreatePortal(World world, int i, int j, int k)
    {
        int l = 0;
        int i1 = 0;
        if(world.getBlockId(i - 1, j, k) == mod_moon.entranceBlock.blockID || world.getBlockId(i + 1, j, k) == mod_moon.entranceBlock.blockID)
        {
            l = 1;
        }
        if(world.getBlockId(i, j, k - 1) == mod_moon.entranceBlock.blockID || world.getBlockId(i, j, k + 1) == mod_moon.entranceBlock.blockID)
        {
            i1 = 1;
        }
        if(l == i1)
        ...
        for(int j1 = -1; j1 <= 2; j1++)
        {
            for(int l1 = -1; l1 <= 3; l1++)
            {
                ...
                if(flag)
                {
                    if(j2 != mod_moon.entranceBlock.blockID)
                    {
                        return false;
                    }
                    continue;
        ...
        for(int k1 = 0; k1 < 2; k1++)
        {
            ...
            {
                world.setBlockWithNotify(i + l * k1, j + i2, k + i1 * k1, mod_moon.moonportal.blockID);
            }
        }
    ...
    public void onNeighborBlockChange(World world, int i, int j, int k, int l)
    {
        ...
        if(world.getBlockId(i, k1 - 1, k) != mod_moon.entranceBlock.blockID)
        {
        ...
        if(l1 != 3 || world.getBlockId(i, k1 + l1, k) != mod_moon.entranceBlock.blockID)
        {
            world.setBlockWithNotify(i, j, k, 0);
            return;
        }
        ...
        if((world.getBlockId(i + i1, j, k + j1) != mod_moon.entranceBlock.blockID || world.getBlockId(i - i1, j, k - j1) != blockID) && (world.getBlockId(i - i1, j, k - j1) != mod_moon.entranceBlock.blockID || world.getBlockId(i + i1, j, k + j1) != blockID))
        {
            world.setBlockWithNotify(i, j, k, 0);
            return;
        } else

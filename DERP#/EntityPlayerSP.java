...
public class EntityPlayerSP extends EntityPlayer
{
    ...
    public void onLivingUpdate()
    {
        ...
        if(inMoonPortal)
        {
            ...
            if(timeInMoonPortal == 0.0F)
            {
                mc.sndManager.playSoundFX("portal.trigger", 1.0F, rand.nextFloat() * 0.4F + 0.8F);
            }
            timeInMoonPortal += 0.0125F;
            if(timeInMoonPortal >= 1.0F)
            {
                timeInMoonPortal = 1.0F;
                if(!worldObj.multiplayerWorld)
                {
        ...
        } else
        {
            if(timeInMoonPortal > 0.0F)
            {
                timeInMoonPortal -= 0.05F;
            }
            if(timeInMoonPortal < 0.0F)
            {
                timeInMoonPortal = 0.0F;
            }
        }
